...
public class UserService {
    ...
    public String getRoleByUserId(Long userId) {
        ...
        if (userOptional.isPresent()) {
            ...
            // Vérifier si l'utilisateur a des rôles associés
            if (user.getRoles() != null && !user.getRoles().isEmpty()) {
                // Puisque chaque utilisateur a un seul rôle, on accède à ce rôle
                return user.getRoles().iterator().next().getName();  // Retourne le nom du rôle
            } else {
                throw new RuntimeException("Aucun rôle trouvé pour l'utilisateur avec ID : " + userId);
            }
        } else {
